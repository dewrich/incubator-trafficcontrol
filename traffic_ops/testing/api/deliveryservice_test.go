...
func CreateTestDeliveryServices(t *testing.T) {
	...
	for _, i := range testData.DeliveryServices {
		resp, err := TOSession.CreateDeliveryService(&i)
		log.Debugln("Response: ", resp)
		if err != nil {
			t.Errorf("could not CREATE ds: %v\n", err)
		}
	}
...
func UpdateTestDeliveryServices(t *testing.T) {
	...
	expectedDeliveryServiceName := "testDS1"
...
func GetTestDeliveryServices(t *testing.T) {
	...
	for _, i := range testData.DeliveryServices {
		resp, _, err := TOSession.GetDeliveryServiceByName(i.Name)
		if err != nil {
			t.Errorf("cannot GET DeliveryService by name: %v - %v\n", err, resp)
		}
	}
