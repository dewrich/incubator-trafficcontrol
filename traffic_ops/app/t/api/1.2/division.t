...
$t->get_ok( Test::TestHelper::TO_URL . '/api/1.2/divisions')->status_is(200)->json_is( "/response/0/id", 100 )
	->json_is( "/response/0/name", "mountain" )->or( sub { diag $t->tx->res->content->asset->{content}; } );

$t->get_ok( Test::TestHelper::TO_URL . '/api/1.2/divisions/100')->status_is(200)->json_is( "/response/0/id", 100 )
	->json_is( "/response/0/name", "mountain" )->or( sub { diag $t->tx->res->content->asset->{content}; } );

ok $t->post_ok( Test::TestHelper::TO_URL . '/api/1.2/divisions' => {Accept => 'application/json'} => json => {
...
ok $t->put_ok( Test::TestHelper::TO_URL . '/api/1.2/divisions/' . $division_id  => {Accept => 'application/json'} => json => {
			"id" => $division_id,
			"name" => "division2"
		})
