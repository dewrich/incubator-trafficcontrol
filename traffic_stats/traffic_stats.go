...
type RunningConfig struct {
	...
	CacheMap        map[string]toapi.Server      // map hostName to cache
		...
		for _, row := range res[0].Series {
			for _, record := range row.Values {
				...
				if record[1] != nil {
					...
					var statsSummary toapi.StatsSummary
...
func writeSummaryStats(config StartupConfig, statsSummary toapi.StatsSummary) {
...
func getToData(config StartupConfig, init bool, configChan chan RunningConfig) {
	...
	runningConfig.CacheMap = make(map[string]toapi.Server)
...
func calcMetrics(cdnName string, url string, cacheMap map[string]toapi.Server, config StartupConfig, runningConfig RunningConfig) {
...
func calcCacheValues(trafmonData []byte, cdnName string, sampleTime int64, cacheMap map[string]toapi.Server, config StartupConfig) error {
