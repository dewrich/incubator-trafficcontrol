...
func GetVitals(newResult *cache.Result, prevResult *cache.Result, mc *toapi.TrafficMonitorConfigMap) {
...
func EvalCache(result cache.ResultInfo, resultStats cache.ResultStatValHistory, mc *toapi.TrafficMonitorConfigMap) (bool, string, string) {
...
func CalcAvailability(results []cache.Result, pollerName string, statResultHistory cache.ResultStatHistory, mc toapi.TrafficMonitorConfigMap, toData todata.TOData, localCacheStatusThreadsafe threadsafe.CacheAvailableStatus, localStates peer.CRStatesThreadsafe, events ThreadsafeEvents) {
...
func exceedsThresholdMsg(stat string, threshold toapi.HealthThreshold, val float64) string {
...
func inThreshold(threshold toapi.HealthThreshold, val float64) bool {
	switch threshold.Comparator {
	case "=":
